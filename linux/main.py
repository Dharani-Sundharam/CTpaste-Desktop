...
class MainWindow(QMainWindow):
    ...
    def register_hotkey(self):
        try:
            # Reformat "alt+v" to pynput's "<alt>+v" format
            hotkey_str = self.hotkey.replace("alt", "<alt>")
            self.hotkey_listener = keyboard.GlobalHotKeys({
                hotkey_str: self.trigger_typing
            })
    ...
    def unregister_hotkey(self):
        if self.hotkey_registered and hasattr(self, 'hotkey_listener'):
            try:
